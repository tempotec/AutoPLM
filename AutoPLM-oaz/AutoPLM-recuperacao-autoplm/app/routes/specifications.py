...
@specifications_bp.route('/fichas')
@login_required
def index():
    """Smart index: show fichas list if any exist, else redirect to upload."""
    ...
    if user.is_admin:
        count = Specification.query.count()
    else:
        count = Specification.query.filter_by(user_id=user.id).count()

    if count == 0:
        return redirect(url_for('specifications.upload'))

    # Has fichas — show list
    if user.is_admin:
        specifications = Specification.query.order_by(Specification.created_at.desc()).all()
        collections = Collection.query.order_by(Collection.name).all()
    else:
        specifications = Specification.query.filter_by(user_id=user.id).order_by(Specification.created_at.desc()).all()
        collections = Collection.query.filter_by(user_id=user.id).order_by(Collection.name).all()

    return render_template('fichas_list.html',
